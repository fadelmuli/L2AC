...
class Actor(object):
    ...
    def __init__(self, sess, n_features, n_actions, lr=0.001):
        ...
        with tf.variable_scope('exp_v'):
            ...
            ppo2loss = tf.minimum(self.r(self.new_pi, self.old_pi, self.acts) * self.td_error, 
                            tf.clip_by_value(self.r(self.new_pi, self.old_pi, self.acts), 1 - 0.2, 1 + 0.2) * self.td_error
                        )

            self.exp_v = tf.reduce_sum(ppo2loss)
            
        with tf.variable_scope('train'):
            self.train_op = tf.train.AdamOptimizer(self.lr).minimize(-self.exp_v)  # minimize(-exp_v) = maximize(exp_v)
                
    def learn(self, s, a, p, p_, td, lr):
        s = s[np.newaxis, :]
        feed_dict = {self.s: s, self.acts: a, self.old_pi: p, self.new_pi: p_, self.td_error: td, self.lr:lr}
